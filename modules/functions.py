...
def title(modules:int):
    ...
    """
    while Checker.checking:
        Checker.title = f"Rhax AIO | Good: {numerize(Checker.good)}  ~  Custom: {numerize(Checker.custom)}  ~  Bad: {numerize(Checker.bad)}  ~  Errors: {numerize(Checker.errors)}  ~  CPM: {numerize(get_cpm())}  ~  Remaining: {numerize(len(Checker.remaining)*modules)}"
...
def title_2():
    ...
    """
    while Checker.checking:
        Checker.title = f"Rhax AIO | Good: {numerize(Checker.good)} ~ Bad: {numerize(Checker.bad)} ~ CPM: {numerize(get_cpm())} ~ Remaining: {numerize(len(Checker.remaining))}"
        change_title(Checker.title)
        sleep(0.1)
